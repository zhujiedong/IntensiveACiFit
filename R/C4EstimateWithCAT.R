#' Estimate photosynthesis parameters for C4 species using Sharkey's 
#' fitting procedure
#'
#' Using the gas exchange measurement (A_Ci curve), 
#' C4 photosynthesis model with
#' carbonic anhydrase reaction and Sharkey et al. (2007) 
#' fitting processure to do nonlinear curve fitting 
#' (using nlminb package) for estimating photosynthesis
#' parameters (Vcmax,J,Rd,gm and Vpmax) for C4 species. 
#' The difference between this method with C4EstimateWithCA 
#' is that temperature response parameters need to be provided
#'  by the users. Thus, this method provides the option
#' to alter temperature response parameters. If only planing 
#' to alter several parameters, not all of them, one can use
#' the other parameters provided by
#' Table S1 in Zhou et al. (2019) ("Deriving C4 photosynthesis 
#' parameters by fitting intensive A/Ci curves"). Make sure to 
#' load the "stats" package before intstalling and using 
#' the "C4Estimation" package.
#' @param ACi     Gas exchange measurement from Li6400 or other 
#' equipment. It is a dataframe iput. Ci with the unit of ppm. 
...
#' @param TresponseKp2 A vector that gives the temperature response parameters for the
#' Kp for HCO3- (c(Kp225,deltaHaKp2))
#' @param Tresponsekr A vector that gives the temperature response parameters for the
#' kr (c(kr25,deltaHakr))
#' @param Tresponsekf A vector that gives the temperature response parameters for the
#' kf (c(kf25,deltaHakf))
#' @param TresponseKH A vector that gives the temperature response parameters for the
#' KH (c(KH25))
#' @param TresponsekCA A vector that gives the temperature response parameters for the
#' kCA (c(deltaHakCA,deltaHdkCA,deltaSkCA))
#' @return This package will return a dataframe that 
...
#' additional information returned by the optimizer, or NULL. 
...
C4EstimateWithCAT = function(
    ...
    TresponseKH = c(2.9799), TresponsekCA = c(40.9, 64.5, 0.21)
    ){
    ...
    Kp225 = TresponseKp2[1]
    deltaHaKp2 = TresponseKp2[2]
    kr25 = Tresponsekr[1]
    deltaHakr = Tresponsekr[2]
    kf25 = Tresponsekf[1]
    deltaHakf = Tresponsekf[2]
    KH25 = TresponseKH[1]
    deltaHakCA = TresponsekCA[1]
    deltaHdkCA = TresponsekCA[2]
    deltaSkCA = TresponsekCA[3]
    ...
    Kp2 = Kp225*exp(deltaHaKp2*(Tleaf-25)/(298.15*0.008314*(273.15+Tleaf)))
    kr=kr25*exp(deltaHakr*(Tleaf-25)/(298.15*0.008314*(273.15+Tleaf)))
    kf=kf25*exp(deltaHakf*(Tleaf-25)/(298.15*0.008314*(273.15+Tleaf)))
    KH=KH25*exp(-2400*(1/(273.15+Tleaf)-1/298.15))
    ...
    Vpmax25<-Parameters[5]/(exp(deltaHaVpmax*(Tleaf-25)/(298.15*0.008314*(273.15+Tleaf)))*
                            ...
                            (1+exp(((273.15+Tleaf)*deltaSVpmax-deltaHdVpmax)/(273.15+Tleaf)/
                                    0.008314)))
    KCA25<-Parameters[6]/(exp(deltaHakCA*(Tleaf-25)/(298.15*0.008314*(273.15+Tleaf)))*
                            (1+exp((298.15*deltaSkCA-deltaHdkCA)/298.15/0.008314))/
                            (1+exp(((273.15+Tleaf)*deltaSkCA-deltaHdkCA)/(273.15+Tleaf)/
                                    0.008314)))

    para25 = data.frame(Vcmax25, J25, Rd25, gm25, Vpmax25, KCA25)
    para = data.frame(Vcmax, J, Rd, gm, Vpmax, KCA)
  ...
  # Explicit calculation of ATE and ATT
    ...
    b_ac = gm*(Ci.obs*gbs+Vpr-Rm)+(x3_ac+x2_ac*O2)*
        gm*gbs+(x1_ac*gammastar+x2_ac*Rd)*gm*alpha1/
        0.047+(gm+gbs)*(x1_ac-Rd)
    b_aj = gm*(Ci.obs*gbs+Vpr-Rm)+(x3_aj+x2_aj*O2)*
        gm*gbs+(x1_aj*gammastar+x2_aj*Rd)*gm*alpha1/
        0.047+(gm+gbs)*(x1_aj-Rd)
