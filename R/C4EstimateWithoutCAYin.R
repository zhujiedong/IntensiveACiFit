...
C4EstimateWithoutCAYin = function(ACi, alpha1 = 0.15, x = 0.4, 
    ...
    Kc=75.06*exp(36.5*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))
    Ko=35.82*exp(12.8*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))
    gammastar=0.000244*exp(24.82*(Tleaf-25)/(298.15*
        0.008314*(273.15+Tleaf)))
    Kp = 8.5455*exp(52.2*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))
    gbs = 0.029455*exp(116.77*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))*(1+exp((298.15*0.86-264.6)/298.15/
        0.008314))/(1+exp(((273.15+Tleaf)*0.86-264.6)/
    ...
    Vcmax25=Parameters[1]/(exp(51.89*(Tleaf-25)/(298.15*
        0.008314*(273.15+Tleaf))))
    J25=Parameters[2]/(exp(69.246*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))*(1+exp((298.15*0.609-188.502)/298.15/
        0.008314))/(1+exp(((273.15+Tleaf)*0.609-188.502)/(273.15
        +Tleaf)/0.008314)))
    Rd25=Parameters[3]/(exp(41.85*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf))))
    gm25=Parameters[4]/(exp(46.533*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))*(1+exp((298.15*1.2-366.8)/298.15/0.008314))/
        (1+exp(((273.15+Tleaf)*1.2-366.8)/(273.15+Tleaf)/0.008314)))
    Vpmax25=Parameters[5]/(exp(65.6905*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))*(1+exp((298.15*0.47-147.69375)/298.15/
        0.008314))/(1+exp(((273.15+Tleaf)*0.47-147.69375)/
        (273.15+Tleaf)/0.008314)))
    ...
    RrPc_p = (d-(x3_aj+x2_aj*O2)*gm*gbs+(Rd-x1_aj)*(gm+gbs)-
        ...
        0.047)/deno_aj
    RcPc_q = (f+(x3_ac+x2_ac*O2)*k+d*(Rd-x1_ac)-gm
        *gbs*(x1_ac*gammastar*O2+Rd*(x3_ac+x2_ac*O2))+
        (x1_ac*gammastar+x2_ac*Rd)*k*alpha1/0.047/gbs)/deno_ac
    RrPc_q = (f+(x3_aj+x2_aj*O2)*k+d*(Rd-x1_aj)-gm*
        gbs*(x1_aj*gammastar*O2+Rd*(x3_aj+x2_aj*O2))+
        (x1_aj*gammastar+x2_aj*Rd)*k*alpha1/0.047/gbs)/deno_aj
    RcPc_r = (Rd*(f+(x3_ac+x2_ac*O2)*k)-x1_ac*(f-k*gammastar*
        O2))/deno_ac
