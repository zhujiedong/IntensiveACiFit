...
#' Rd,gm and Vpmax) for C4 species using Yin's explicit equations. The difference
#' between this method with C4EstimateWithoutCAYin is that temperature response
...
#' A vector of (c(Vcmax,J,Rd,gm and Vpmax))
...
C4EstimateWithoutCAYinT = function(
    ...
    fn=function(Param){
        ...
        RrPc_p = (d-(x3_aj+x2_aj*O2)*gm*gbs+(Rd-x1_aj)*(gm+gbs)-
            (x1_aj*gammastar*gm+x2_aj*Rd*gm-x2_aj*k/gbs)*alpha1/
            0.047)/deno_aj
        ...
        RrPc_q = (f+(x3_aj+x2_aj*O2)*k+d*(Rd-x1_aj)-
            ...
            deno_aj
        RcPc_r = (Rd*(f+(x3_ac+x2_ac*O2)*k)-x1_ac*(f-k*gammastar*O2))/
            deno_ac
        RrPc_r = (Rd*(f+(x3_aj+x2_aj*O2)*k)-x1_aj*(f-k*gammastar*O2))/
            deno_aj
        ...
        sum((A.obs-(Ci.obs<=CaBreakL)*Ac-(Ci.obs>=CaBreakH)*
            Aj-(Ci.obs>CaBreakL)*(Ci.obs<CaBreakH)*((Ac<Aj)*
            Ac+(Ac>=Aj)*Aj))^2)
    }
    ...
    Est.model = nlminb(c(startp[1],startp[2] , startp[3], startp[4],
        startp[5]),fn, lower=c(0,0,0,0,0), upper=c(200, 600, 20, 30, 200))
    ...
    Vcmax25=Parameters[1]/(exp(deltaHaVcmax*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf))))
    J25=Parameters[2]/(exp(deltaHaJ*(Tleaf-25)/(298.15*0.008314*(273.15+
        Tleaf)))*(1+exp((298.15*deltaSJ-deltaHdJ)/298.15/0.008314))/
        (1+exp(((273.15+Tleaf)*deltaSJ-deltaHdJ)/(273.15+Tleaf)/
        0.008314)))
    Rd25=Parameters[3]/(exp(deltaHaRd*(Tleaf-25)/(298.15*0.008314*(273.15+
        Tleaf))))
    gm25=Parameters[4]/(exp(deltaHagm*(Tleaf-25)/(298.15*0.008314*(273.15+
        Tleaf)))*(1+exp((298.15*deltaSgm-deltaHdgm)/298.15/0.008314))/
        (1+exp(((273.15+Tleaf)*deltaSgm-deltaHdgm)/(273.15+Tleaf)/
        0.008314)))
    Vpmax25=Parameters[5]/(exp(deltaHaVpmax*(Tleaf-25)/(298.15*0.008314*
        (273.15+Tleaf)))*(1+exp((298.15*deltaSVpmax-deltaHdVpmax)/
        298.15/0.008314))/(1+exp(((273.15+Tleaf)*deltaSVpmax-deltaHdVpmax)/
        (273.15+Tleaf)/0.008314)))
